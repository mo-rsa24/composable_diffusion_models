# visualize_composition_latent_ddim.py
import torch
import numpy as np
import os
...
# Using the same stable schedule as the Itô version for consistency
...
set_seed(42)
N_SAMPLES = 512
N_STEPS = 200  # DDIM is efficient and needs fewer steps
...
x_gen_history = []  # (MODIFIED) Use a list for guaranteed ordering and size
with torch.no_grad():
    x = torch.randn(N_SAMPLES, 2, device=DEVICE)
    time_steps = torch.linspace(1.0, 1e-3, N_STEPS + 1, device=DEVICE)

    # (MODIFIED) Calculate a fixed interval to save 5 snapshots during the process
    save_interval = N_STEPS // 5

    for i in trange(N_STEPS, desc="Generating with DDIM Sampler"):
        # (MODIFIED) Save history at fixed intervals to get 5 snapshots
        if i % save_interval == 0:
            x_gen_history.append(x.cpu().numpy())

        t_now = time_steps[i]
        t_next = time_steps[i + 1]
        t_tensor = torch.full((N_SAMPLES,), t_now, device=DEVICE)

        # --- Heuristic Composition Step ---
        ...
        x0_pred.clamp_(-20, 20)  # Clamp based on typical latent space scale
    ...
    x_gen_final = x
    # (MODIFIED) Add the final state to get the 6th snapshot
    x_gen_history.append(x_gen_final.cpu().numpy())

# --- 4. Plot the Latent Space Reverse Process ---
all_plot_points = x_gen_history.copy()
...
# (MODIFIED) Define plot times based on the 6 snapshots we saved
plot_times = np.linspace(1.0, 0.0, 6)

for i, t_val in enumerate(plot_times):
    ax = axes[i]
    # Get the corresponding ground truth noise level for visualization
    ...
    # (MODIFIED) Get the generated data by index from our list
    xt_gen = x_gen_history[i]
...
axes[0].legend()
fig.suptitle("Reverse Diffusion using Combined Latent Models (DDIM Method)", fontsize=16)
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig(os.path.join(OUTPUT_DIR, "latent_composition_process_shapes_ddim.png"))
plt.close()
print(f"Latent visualization saved to {os.path.join(OUTPUT_DIR, 'latent_composition_process_shapes_ddim.png')}")
...
save_image(img_reconstructed.clamp(-1, 1), os.path.join(OUTPUT_DIR, "reconstructed_shapes_ddim.png"), nrow=16,
           normalize=True, value_range=(-1, 1))
print(f"Reconstructed images saved to {os.path.join(OUTPUT_DIR, 'reconstructed_shapes_ddim.png')}")
