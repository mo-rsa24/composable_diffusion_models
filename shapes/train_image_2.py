...
def sample_full_ddim(model, num_samples, num_classes, device, img_size, in_channels, timesteps=100):
    """
    Performs a full reverse diffusion process using a DDIM deterministic sampler.
    This function was already correct and remains a valid way to generate samples for validation.
    """
    model.eval()
...
def main(args):
    """
    The main training loop. The logic here was already faithful to standard diffusion
    model training (noise prediction objective) and did not require changes.
    The key fix was ensuring it uses the corrected, consistent schedule.
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"
    set_seed(42)
    ...
    for epoch in tqdm(range(epochs), desc=f"Training {args.training_mode} model"):
        for batch in dataloader:
            ...
            # Sample a random time t for each image in the batch
            ...
            if (epoch + 1) % val_log_every_epoch == 0:
                ...
                with torch.no_grad():
                    ...
                    save_grid(x0_hat, str(training_mode_dir / f"epoch_{epoch + 1}_onestep_val.png"), in_channels)
                model.train()
            ...
            if (epoch + 1) % log_every_epoch == 0:
                ...
                training_mode_dir = output_dir / args.training_mode
                training_mode_dir.mkdir(parents=True, exist_ok=True)
                save_grid(full_samples, str(training_mode_dir / f"epoch_{epoch + 1}_full_val.png"), in_channels)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sanity", action='store_true', help="Run sanity checks to ensure that the model is running")
