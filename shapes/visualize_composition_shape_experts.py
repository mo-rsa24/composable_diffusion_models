# visualize_composition_shape_experts.py
import torch
import numpy as np
import os
...
# Import beta(t) for the JAX-faithful ODE solver
...
def vector_field(model, t, x):
    """
    Computes the model output (eps_hat) and its divergence using
    a Rademacher-based Hutchinson's estimator to match the JAX reference.
    """
    x_clone = x.clone().requires_grad_(True)
    t_in = torch.full((x.shape[0],), t, device=x.device)
    eps_hat = model(t_in, x_clone)

    # Use Rademacher {-1, 1} vectors for Hutchinson's estimator to match JAX
...
# --- Paths ---
OUTPUT_DIR = "outputs_shapes_3/composition_shapes"
CHECKPOINT_DIR = "checkpoints_shapes_3"
# Load the two specialist shape models
...
print("Loading specialist models and PCA...")
# model_up will be the circle expert (label 0)
model_up = MLP(num_out=2).to(DEVICE).eval()
# model_down will be the square expert (label 1)
...
for i in trange(N_STEPS, desc="Generating with Shape Expert Composition"):
    t_val = 1.0 - i * dt
...
axes[0].legend()
fig.suptitle("Superposition of Shape Experts (Circle AND Square)", fontsize=16)
plt.tight_layout(rect=[0, 0.03, 1, 0.95])
plt.savefig(os.path.join(OUTPUT_DIR, "latent_composition_shape_experts.png"))
plt.close()
print(f"Latent visualization saved to {os.path.join(OUTPUT_DIR, 'latent_composition_shape_experts.png')}")
...
save_image(img_reconstructed.clamp(-1, 1), os.path.join(OUTPUT_DIR, "reconstructed_images_shape_experts.png"), nrow=16,
           normalize=True, value_range=(-1, 1))
print(f"Reconstructed images saved to {os.path.join(OUTPUT_DIR, 'reconstructed_images_shape_experts.png')}")
