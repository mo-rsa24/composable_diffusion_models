...
def main(args):
    ...
    print("Generating a grid of all shape/color compositions using DDIM...")
    ...
    grid_path = os.path.join(args.output_dir, "composition_grid_ddim.png")
    save_image(grid, grid_path, nrow=len(Config.COLORS) * args.bs, normalize=True, value_range=(-1, 1))
    print(f"\nSaved DDIM generation grid to {grid_path}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Compose shape and color diffusion models in pixel space using DDIM.")
    ...
    parser.add_argument('--output_dir', type=str, default="composition_output_images_ddim")
