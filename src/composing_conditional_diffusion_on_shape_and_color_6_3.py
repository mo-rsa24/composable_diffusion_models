...
class Config:
    ...
    OUTPUT_DIR = f"src/scripts/mini-experiments/visualizations/{EXP_NAME}/composable_diffusion_output_part_6_3_3"
...
def sample_superdiff(model1, model2, label1_idx, label2_idx, mode='OR', T=1.0, l=0.0):
    ...
    for i in tqdm(reversed(range(0, Config.TIMESTEPS)), desc=f"SUPERDIFF ({mode}) Sampling", total=Config.TIMESTEPS):
        ...
        shape_weight = 0.7  # Tune this value (e.g., 0.6, 0.7)
        color_weight = 1.0 - shape_weight
        kappa = torch.tensor([shape_weight, color_weight], device=Config.DEVICE)
