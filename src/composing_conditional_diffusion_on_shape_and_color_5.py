...
if __name__ == '__main__':
    ...
    for s_name in Config.SHAPES:
        for c_name in Config.COLORS:
            ...
            generated_image = sample_composed(shape_model, color_model, s_idx, c_idx, w_shape=w_shape, w_color=w_color)
    ...
    output_path = os.path.join(Config.OUTPUT_DIR, "compositional_generation_grid_paper_method.png")
    ...
    try:
        ...
        plt.imshow(grid_pil)
        plt.axis('off')
        plt.title("Compositional Generation (Paper's Method)\n(Rows: Shape, Cols: Color)")
