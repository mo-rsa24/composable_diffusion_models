...
@torch.no_grad()
# MODIFICATION: Add l=0.0 as a parameter to accept the bias
def sample_superdiff(model1, model2, label1_idx, label2_idx, mode='OR', T=1.0, l=0.0):
    ...
    for i in tqdm(reversed(range(0, Config.TIMESTEPS)), desc=f"SUPERDIFF ({mode}) Sampling", total=Config.TIMESTEPS):
        # Using the corrected logic from the previous step
        t = torch.full((1,), i, device=Config.DEVICE, dtype=torch.long)
        ...
        if mode == 'OR':
            ...
            dW_noise = torch.randn_like(prev_img) * torch.sqrt(torch.tensor(d_tau, device=Config.DEVICE))
            ...
            # MODIFICATION: Add the bias 'l' to the B_vec calculation
            # This implements d_log_q_shape = d_log_q_color + l
            B_vec = torch.tensor([(b[1] - b[0] + l).item(), 1.0], device=Config.DEVICE)

            try:
                kappa = torch.linalg.solve(A_mat, B_vec)
                kappa = torch.clamp(kappa, min=0, max=1.0)
                if torch.sum(kappa) > 0:
