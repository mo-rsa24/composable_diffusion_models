...
def sample_superdiff(shape_model, color_model, shape_idx, color_idx, mode='OR', T=1.0, l=0.0):
    """
    Sample using the SUPERDIFF algorithm from the paper.
    Supports 'OR' and 'AND' modes.
    """
    ...
    for i in tqdm(reversed(range(0, Config.TIMESTEPS)), desc=f"SUPERDIFF ({mode}) Sampling", total=Config.TIMESTEPS):
        ...
        kappa = torch.zeros(2, device=Config.DEVICE)
        if mode == 'OR':
            # OR operation: softmax of log-densities [cite: 971]
            log_q_tensor = torch.stack(log_qs)
            kappa = F.softmax(T * log_q_tensor + l, dim=0)

        elif mode == 'AND':
            # AND operation: solve linear equations for kappa [cite: 971, 1018]
            # Based on Proposition 6 and Appendix C.1
            ...
            uncomposed_drifts = [-f_t + 0.5 * g_t_sq * s for s in scores]

            # System of equations: A * k = B
            # (a1 - a2)k_1 = b2 - b1
            # k1 + k2 = 1
            a = torch.zeros(2, 2, device=Config.DEVICE)
            for r in range(2):  # row (model i)
                for c in range(2):  # col (model j)
                    a[r, c] = d_tau * torch.sum(uncomposed_drifts[c] * scores[r])

            div_f = f_t_coeff * d  # Divergence of f_t(x) = c*x is c*dim(x)
            b = torch.zeros(2, device=Config.DEVICE)
            for r in range(2):
                term1 = div_f
                term2 = torch.sum((f_t - 0.5 * g_t_sq * scores[r]) * scores[r])
                b[r] = d_tau * (term1 + term2)

            # We want d_log_q1 = d_log_q2, where d_log_qi = (a_i . k) + b_i + noise_term
            # (a1-a2) . k = b2 - b1
            A_mat = torch.tensor([
            ...
            try:
                ...
                # Clamp kappa to be non-negative, and re-normalize
                kappa = torch.clamp(kappa, min=0)
        ...
        # --- 2. Calculate Composed Noise and Denoise ---
        pred_noises = [m(img, t, lab) for m, lab in zip(models, labels)]
        composed_noise = kappa[0] * pred_noises[0] + kappa[1] * pred_noises[1]

        # Standard DDPM denoising step using the composed noise
        ...
        # --- 3. Update Log Densities using Itô Estimator (Theorem 1) ---
        dx = img - prev_img
        d_tau = 1.0 / Config.TIMESTEPS
        ...
        for idx in range(2):
            score_i = -pred_noises[idx] / extract(sqrt_one_minus_alphas_cumprod, t, pred_noises[idx].shape)
            f_t_i = f_t_coeff * prev_img
            g_t_sq_i = betas_t  # g^2 is related to beta in DDPM

            term1 = torch.sum(dx * score_i)
            term2_inner_dot = torch.sum((f_t_i - 0.5 * g_t_sq_i * score_i) * score_i)
