...
import math
import os
from src.utils.tools import CheckpointManager, save_config_to_yaml, tiny_subset, is_cluster
...
class ShapesDataset(Dataset):
    ...
    def __init__(self,cfg, size=1000, img_size=64, shapes=None, colors=None, holdout=None, train=True):
        self.size = size
        self.img_size = img_size
        self.shapes = shapes if shapes is not None else cfg.experiment.shapes
        self.colors = colors if colors is not None else cfg.experiment.colors
...
class Sampler:
    ...
    @torch.no_grad()
    def sample_image(self, cfg, model: SimpleUnet, labels, device):
        ...
        for i in tqdm(reversed(range(0, cfg.diffusion.timesteps)), desc="Diffusion Sampling",
                      total=cfg.diffusion.timesteps):
...
def train_model(cfg, model, sampler: Sampler, dataloader, optimizer, condition_type, model_name, device, ckpt_mgr, results_dir,  debug: bool = False):
    ...
    for epoch in range(1, cfg.training.epochs + 1):
        ...
        if epoch % cfg.training.log_every_epoch == 0:
            model.eval()
            generated_image = sampler.sample_image(cfg, model, labels, device)
...
@torch.no_grad()
def sample_composed(cfg, shape_model, color_model, sampler:Sampler, shape_idx, color_idx, device, w_shape=1.0, w_color=1.0):
    """Sample using the composed scores of the two models."""
    print(
        f"Sampling with composition: {cfg.experiment.shapes[shape_idx]} ({w_shape:.1f}) + {cfg.experiment.colors[color_idx]} ({w_color:.1f})")

    # Start from pure noise
    img_size = cfg.dataset.image_size
    img = torch.randn((1, 3, img_size, img_size), device=device)

    shape_label = torch.full((1,), shape_idx, device=device, dtype=torch.long)
...
def main(args):
    ...
    dataset = ShapesDataset(cfg, size=5000, holdout=HOLDOUT_COMBINATION, train=True)
    if args.sanity:
    ...
    # 2. Initialize Models
    shape_model = SimpleUnet(num_classes=len(cfg.experiment.shapes)).to(device)
    color_model = SimpleUnet(num_classes=len(cfg.experiment.colors)).to(device)

    # 3. Train Models
    shape_optimizer = torch.optim.Adam(shape_model.parameters(), lr=cfg.optimizer.params.get('lr'))
    color_optimizer = torch.optim.Adam(color_model.parameters(), lr=cfg.optimizer.params.get('lr'))
    sampler = Sampler(cfg, device=device)
    model_A_name = f"Shape"
    model_B_name = f"Color"


    train_model(cfg, shape_model, sampler, dataloader, shape_optimizer, model_A_name,'shape', device, ckpt_mgr, results_dir)
    ...
    shape_map = {name: i for i, name in enumerate(cfg.experiment.shapes)}
    color_map = {name: i for i, name in enumerate(cfg.experiment.colors)}

    # Generate an image for every possible combination
    generated_images = []
    for s_name in cfg.experiment.shapes:
        for c_name in cfg.experiment.colors:
            s_idx = shape_map[s_name]
            c_idx = color_map[c_name]

            # Note if this combination was held out
            if (s_name, c_name) == HOLDOUT_COMBINATION:
                print(f"\nGenerating HELD-OUT combination: {s_name}, {c_name}")
            else:
                print(f"\nGenerating seen combination: {s_name}, {c_name}")

            # Sample using composition
            generated_image = sample_composed(cfg, shape_model, color_model, sampler, s_idx, c_idx, device, w_shape=1.0, w_color=1.0)
            generated_images.append(generated_image)

    # Save the results in a grid
    grid = make_grid(torch.cat(generated_images), nrow=len(cfg.experiment.colors), normalize=True, value_range=(-1, 1))
    ...
    try:
        ...
        for i, shape in enumerate(cfg.experiment.shapes):
            plt.text(-20, (i + 0.5) * cfg.dataset.image_size, shape, ha='center', va='center', rotation=90, fontsize=12)
        for i, color in enumerate(cfg.experiment.colors):
