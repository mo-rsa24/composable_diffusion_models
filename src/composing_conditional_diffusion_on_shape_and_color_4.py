...
class ShapesDataset(Dataset):
    ...
    def __init__(self, size=1000, img_size=64, shapes=None, colors=None, holdout=None, train=True):
        ...
        self.bbox_colors = Config.BBOX_COLORS
        self.bbox_color_to_idx = {c: i for i, c in enumerate(self.bbox_colors)}
    ...
    def _draw_bounding_box(self, draw, color_name, padding=4):
        """Helper function to draw a colored bounding box."""
        img_size = self.img_size
        draw.rectangle(
            [(padding, padding), (img_size - padding, img_size - padding)],
            outline=color_name,
            width=2  # Adjust the bounding box width as needed
        )
    def __getitem__(self, idx):
        ...
        bbox_color_name = self.bbox_colors[idx % len(self.bbox_colors)]
        bbox_color_idx = self.bbox_color_to_idx[bbox_color_name]
        self._draw_bounding_box(draw, bbox_color_name)

        shape_idx = self.shape_to_idx[shape_name]
        color_idx = self.color_to_idx[color_name]

        return self.transforms(image), torch.tensor(shape_idx), torch.tensor(color_idx), torch.tensor(bbox_color_idx)
...
def train_model(model, dataloader, optimizer, num_epochs, condition_type, debug: bool = False):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (images, shape_labels, color_labels, bbox_color_labels) in enumerate(progress_bar):
            optimizer.zero_grad()
            ...
            elif condition_type == 'color':
                labels = color_labels.to(Config.DEVICE)
            else:  # 'bbox'
                labels = bbox_color_labels.to(Config.DEVICE)
...
@torch.no_grad()
def sample_composed(shape_model, color_model, bbox_model, shape_idx, color_idx, bbox_idx, w_shape=1.0, w_color=1.0, w_bbox=1.0):
    """Sample using the composed scores of the two models."""
    shape_name = Config.SHAPES[shape_idx]
    color_name = Config.COLORS[color_idx]
    bbox_color_name = Config.BBOX_COLORS[bbox_idx]

    print(
        f"Sampling with composition: {shape_name} ({w_shape:.1f}) + {color_name} ({w_color:.1f}) + BBox: {bbox_color_name} ({w_bbox:.1f})")
    ...
    bbox_label = torch.full((1,), bbox_idx, device=Config.DEVICE, dtype=torch.long)
    ...
    for i in tqdm(reversed(range(0, Config.TIMESTEPS)), desc="Composed Sampling", total=Config.TIMESTEPS):
        ...
        pred_noise_bbox = bbox_model(img, t, bbox_label)
        ...
        composed_noise = (w_shape * pred_noise_shape + w_color * pred_noise_color + w_bbox * pred_noise_bbox) / (
                    w_shape + w_color + w_bbox)
...
if __name__ == '__main__':
    ...
    bbox_model = SimpleUnet(num_classes=len(Config.BBOX_COLORS)).to(Config.DEVICE)
    ...
    bbox_optimizer = torch.optim.Adam(bbox_model.parameters(), lr=Config.LR)
    ...
    train_model(bbox_model, dataloader, bbox_optimizer, Config.NUM_EPOCHS, 'bbox')
    ...
    bbox_color_map = {name: i for i, name in enumerate(Config.BBOX_COLORS)}

    # Generate an image for every possible combination
    generated_images = []
    for s_name in Config.SHAPES:
        for c_name in Config.COLORS:
            b_idx = len(generated_images) % len(Config.BBOX_COLORS)
            ...
            generated_image = sample_composed(shape_model, color_model, bbox_model, s_idx, c_idx, b_idx, w_shape=1.0,
                                              w_color=1.0, w_bbox=1.0)
